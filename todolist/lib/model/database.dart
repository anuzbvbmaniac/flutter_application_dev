...
// to import database.g.dart run $ flutter packages pub run build_runner build in terminal.
part 'database.g.dart';

@UseMoor(
  tables: [Todo],
  queries: {
    '_getByType' : 'SELECT * FROM todo WHERE todo_type = ? ORDER BY is_complete, date, time',
    '_completeTask' : 'UPDATE todo SET is_complete = 1 WHERE id = ?',
...
class Database extends _$Database with ChangeNotifier{
  ...
  Database() : super(FlutterQueryExecutor.inDatabaseFolder(path: 'anuztodotask.sqlite'));
